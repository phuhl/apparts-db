...
class Transaction {
    ...
    _decideOperator(key, op, val, newVals) {
        switch (op) {
            ...
            case "in":
                ...
                return (`"${key}" IN (` + val.map(() => `$${this._counter++}`).join(",") + ")");
            case "of":
                if (val.path.length < 1) {
                    throw new Error("ERROR, operator 'of' requires at least one path element. You submitted [].");
                }
    ...
    insert(content, returning = ["id"]) {
        ...
        const params = [].concat.apply([], content.map((c) => keys.map((k) => Array.isArray(c[k]) ? this._transformArray(c[k]) : c[k])));
        return this._dbs
            .query(q, params)
            .then((res) => {
            return Promise.resolve(res.rows);
